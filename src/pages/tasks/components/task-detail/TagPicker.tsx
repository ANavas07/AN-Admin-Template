    ...
    return (
        <div className="relative">
            <div className="flex flex-wrap items-center gap-1.5">
                {appliedTags.map((tag) => (
                    <span key={tag.id} className={`inline-flex items-center gap-1 rounded-md border px-1.5 py-0.5 text-2xs font-medium ${accentStyles[tag.color]}`}>
                        {tag.label}
                        <button
                            type="button"
                ...
                <button
                    ...
                    aria-expanded={open}
                    className="inline-flex items-center gap-1 rounded-md border border-dashed border-line px-1.5 py-0.5 text-2xs font-medium text-fg-muted hover:border-brand hover:text-brand"
                >
            ...
            {open ? (
                <>
                    ...
                    <div role="dialog" className="absolute left-0 top-8 z-50 w-64 rounded-xl border border-line bg-surface p-2 shadow-lg">
                            ...
                            {catalog.map((tag) => {
                                ...
                                return (
                                    <button
                                        ...
                                        type="button"
                                        onClick={() => toggle(tag)}
                                        className="flex w-full items-center justify-between gap-2 rounded-lg px-2 py-1.5 text-left hover:bg-canvas-subtle"
                                    >
                                        <span className="flex items-center gap-2">
                                            <span className={`h-2.5 w-2.5 rounded-full ${accentDot[tag.color]}`} aria-hidden="true" />
                                            <span className="text-sm text-fg">{tag.label}</span>
                                        </span>
                                        {applied ? (
                        ...
                        <div className="mt-2 border-t border-line pt-2">
                            <p className="mb-1.5 text-2xs font-semibold uppercase tracking-caps text-fg-muted">Crear etiqueta</p>
                            <input
                                value={draft}
                                onChange={(e) => setDraft(e.target.value)}
                                onKeyDown={(e) => { if (e.key === 'Enter') create() }}
                                placeholder="Nombre de la etiqueta"
                                className="mb-2 w-full rounded-lg border border-line bg-surface px-2 py-1.5 text-sm text-fg placeholder:text-fg-muted focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand/25"
                            />
                                ...
                                <div className="flex items-center gap-1">
                                    {colorChoices.map((color) => (
                                        <button
                                            ...
                                            className={`h-5 w-5 rounded-full ${accentDot[color]} ${draftColor === color ? 'ring-2 ring-offset-1 ring-fg ring-offset-surface' : ''}`}
                                        />
                                    ))}
                                </div>
                                <button
                                    ...
                                    onClick={create}
                                    disabled={!draft.trim()}
                                    className="rounded-md bg-brand-solid px-2.5 py-1 text-xs font-medium text-on-solid disabled:opacity-40"
                                >
                                    Crear
                                </button>
                            </div>
